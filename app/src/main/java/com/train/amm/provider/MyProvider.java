package com.train.amm.provider;
...
import com.train.amm.MyOpenHelper;
...
public class MyProvider extends ContentProvider {
    ...
    //检测用户传入的uri和匹配器定义好的uri中哪个匹配
    static {
        uriMatcher.addURI("com.train.pprovider", "person", 1);
        uriMatcher.addURI("com.train.pprovider", "student", 2);
        //路径后还可以携带数据
        uriMatcher.addURI("com.train.pprovider", "person/#", 3);
    }
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        ...
        if (uriMatcher.match(uri) == 1) {
            cursor = writableDatabase.query("person", projection, selection, selectionArgs, null, null, sortOrder, null);
        } else if (uriMatcher.match(uri) == 2) {
            cursor = writableDatabase.query("student", projection, selection, selectionArgs, null, null, sortOrder, null);
        } else if (uriMatcher.match(uri) == 3) {
            long id = ContentUris.parseId(uri);
            cursor = writableDatabase.query("person", projection, "_id =?", new String[]{id + ""}, null, null, sortOrder, null);
        } else {
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        if (uriMatcher.match(uri) == 1) {
            ...
            //发送数据改变的通知
            //uri通知发动到哪个uri上,所有注册在这个uri上的内容观察者都可以得到通知
            getContext().getContentResolver().notifyChange(uri, null);
        } else if (uriMatcher.match(uri) == 2) {
            writableDatabase.insert("student", null, values);
            getContext().getContentResolver().notifyChange(uri, null);
        } else {
            throw new IllegalArgumentException("uri地址错误");
        }
        return uri;
    }
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        int person = writableDatabase.delete("person", selection, selectionArgs);
        return person;
    }
